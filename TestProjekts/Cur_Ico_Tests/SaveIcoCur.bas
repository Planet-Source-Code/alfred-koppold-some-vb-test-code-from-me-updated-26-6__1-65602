...
Else
cb.bColorCount = 0 'Nur 1 Bit steht zur Verf¸gung (nicht unbedingt notwendig)
End If
cb.bHeight = Dateiinhalt(Nummer).HˆhePixel
cb.bReserved = 0
cb.bWidth = Dateiinhalt(Nummer).BreitePixel

cb.dwBytesinRes = GrGesamt 'GrGesamt 'Komplettgrˆﬂe
...
bmi.biClrUsed = 0
bmi.biCompression = 0
bmi.biHeight = Dateiinhalt(Nummer).HˆhePixel * 2
bmi.biPlanes = 1
bmi.biSize = 40
